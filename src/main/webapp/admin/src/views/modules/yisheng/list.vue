<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                  ...
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.yishengxingming" placeholder="医生姓名" clearable></el-input>
                                                                                                    ...
                                                                                                    <el-form-item :label="contents.inputTitle == 1 ? '所在医院' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.suozaiyiyuan" placeholder="所在医院" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.suozaiyiyuan" placeholder="所在医院" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.suozaiyiyuan" placeholder="所在医院" clearable></el-input>
                </el-form-item>
                                                                              <el-form-item :label="contents.inputTitle == 1 ? '所在部门' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.suozaibumen" placeholder="所在部门" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.suozaibumen" placeholder="所在部门" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.suozaibumen" placeholder="所在部门" clearable></el-input>
                </el-form-item>
                                                                              <el-form-item :label="contents.inputTitle == 1 ? '预约星期段' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.yuyuexingqiduan" placeholder="预约星期段" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.yuyuexingqiduan" placeholder="预约星期段" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.yuyuexingqiduan" placeholder="预约星期段" clearable></el-input>
                </el-form-item>
                                                                            <el-form-item :label="contents.inputTitle == 1 ? '预约时间段' : ''">
                  <el-date-picker
                      v-model="searchForm.yuyueshijianduanstart"
                      type="date"
                      placeholder="预约时间段">
                  </el-date-picker>
                  至
                  <el-date-picker
                      v-model="searchForm.yuyueshijianduanend"
                      type="date"
                      placeholder="预约时间段">
                  </el-date-picker>
              </el-form-item>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="所在部门">
		     <template slot-scope="scope">
                       {{scope.row.suozaibumen}}
                     </template>
                </el-table-column>
                	                	                                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="yuyuexingqiduan"
                    header-align="center"
		    label="预约星期段">
		     <template slot-scope="scope">
                       {{scope.row.yuyuexingqiduan}}
                     </template>
                </el-table-column>
                	                	                                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
		    ...
		    label="联系电话">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('yisheng','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('yisheng','预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yuyueCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'预约':'' }}</el-button>
                <el-button v-if="isAuth('yisheng','预约') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yuyueCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'预约':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yisheng','预约') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yuyueCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'预约':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <yuyue-cross-add-or-update v-if="yuyueCrossAddOrUpdateFlag" :parent="this" ref="yuyueCrossaddOrUpdate"></yuyue-cross-add-or-update>
...
import yuyueCrossAddOrUpdate from "../yuyue/add-or-update";
export default {
  data() {
    return {
                                                                                                            suozaibumenOptions: [],
      ...
      addOrUpdateFlag:false,
                  yuyueCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            yuyueCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yuyueCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yisheng');
      this.$nextTick(() => {
        this.$refs.yuyueCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                              this.$http({
            url: `option/fenlei/bumenfenlei`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.suozaibumenOptions = data.data;
            } else {
      ...
      let params = {
        ...
        sort: 'id',
            ...
            params['yishengxingming'] = '%' + this.searchForm.yishengxingming + '%'
                                                                                  ...
                                                                                  if(this.searchForm.suozaiyiyuan!='' && this.searchForm.suozaiyiyuan!=undefined){
            params['suozaiyiyuan'] = '%' + this.searchForm.suozaiyiyuan + '%'
          }
                                                            if(this.searchForm.suozaibumen!='' && this.searchForm.suozaibumen!=undefined){
            params['suozaibumen'] = '%' + this.searchForm.suozaibumen + '%'
          }
                                                            if(this.searchForm.yuyuexingqiduan!='' && this.searchForm.yuyuexingqiduan!=undefined){
            params['yuyuexingqiduan'] = '%' + this.searchForm.yuyuexingqiduan + '%'
          }
                                                            if(this.searchForm.yuyueshijianduanstart!='' && this.searchForm.yuyueshijianduanstart!=undefined ){
            params['yuyueshijianduanstart'] = this.searchForm.yuyueshijianduanstart
          }
          if(this.searchForm.yuyueshijianduanend!='' && this.searchForm.yuyueshijianduanend!=undefined){
            params['yuyueshijianduanend'] = this.searchForm.yuyueshijianduanend
          }
                                                                                                this.$http({
        url: "yisheng/page",
        method: "get",
