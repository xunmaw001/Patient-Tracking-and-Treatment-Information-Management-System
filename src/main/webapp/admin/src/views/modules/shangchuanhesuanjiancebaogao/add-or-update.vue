          ...
          <el-form-item class="input" label="手机" prop="shouji">
              <el-input v-model="ruleForm.shouji" 
                placeholder="手机" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="核算检测报告" prop="hesuanjiancebaogao">
          <file-upload
          tip="点击上传核算检测报告"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.hesuanjiancebaogao?ruleForm.hesuanjiancebaogao:''"
          @change="hesuanjiancebaogaoUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.hesuanjiancebaogao" label="核算检测报告" prop="hesuanjiancebaogao">
            <el-button type="text" size="small" @click="download(ruleForm.hesuanjiancebaogao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
                              </el-row>
...
export default {
  data() {
	...
	hesuanjiancebaogao : false,
            ...
            ruleForm: {
                  ...
                  hesuanjiancebaogao: [
                            { required: true, message: '核算检测报告不能为空', trigger: 'blur' },
                                    	                                                              ],
              }
    };
  },
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shouji = true;
            continue;
	            	            ...
	            	            if(o=='hesuanjiancebaogao'){
            this.ruleForm.hesuanjiancebaogao = obj[o];
	    this.ro.hesuanjiancebaogao = true;
            continue;
          }
	                    }
                                                                              }
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                        ...
                                        // 多级联动参数
                                                    info(id) {
      this.$http({
        url: `shangchuanhesuanjiancebaogao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `shangchuanhesuanjiancebaogao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangchuanhesuanjiancebaogaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
                                                            ...
                                                            hesuanjiancebaogaoUploadChange(fileUrls) {
                this.ruleForm.hesuanjiancebaogao = fileUrls;
				this.addEditUploadStyleChange()
            },
            	addEditStyleChange() {
	  this.$nextTick(()=>{
