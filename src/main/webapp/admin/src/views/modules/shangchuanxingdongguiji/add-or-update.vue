          ...
          <el-form-item class="input" label="手机" prop="shouji">
              <el-input v-model="ruleForm.shouji" 
                placeholder="手机" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="上传出行报告" prop="shangchuanchuxingbaogao">
          <file-upload
          tip="点击上传上传出行报告"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.shangchuanchuxingbaogao?ruleForm.shangchuanchuxingbaogao:''"
          @change="shangchuanchuxingbaogaoUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.shangchuanchuxingbaogao" label="上传出行报告" prop="shangchuanchuxingbaogao">
            <el-button type="text" size="small" @click="download(ruleForm.shangchuanchuxingbaogao)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
                              </el-row>
...
export default {
  data() {
	...
	xingming : false,
	shouji : false,
	shangchuanchuxingbaogao : false,
      },
            ruleForm: {
                	        zhanghao: '',
	                        	        xingming: '',
	                        	        shouji: '',
	                        	        shangchuanchuxingbaogao: '',
	                      },
                                                                    rules: {
                  zhanghao: [
                                    	                                                              ],
                  ...
                  shangchuanchuxingbaogao: [
                            { required: true, message: '上传出行报告不能为空', trigger: 'blur' },
                                    	                                                              ],
              }
    };
  },
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shouji = true;
            continue;
	            	            ...
	            	            if(o=='shangchuanchuxingbaogao'){
            this.ruleForm.shangchuanchuxingbaogao = obj[o];
	    this.ro.shangchuanchuxingbaogao = true;
            continue;
          }
	                    }
                                                                              }
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                        ...
                                        // 多级联动参数
                                                    info(id) {
      this.$http({
        url: `shangchuanxingdongguiji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `shangchuanxingdongguiji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shangchuanxingdongguijiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
                                                            ...
                                                            shangchuanchuxingbaogaoUploadChange(fileUrls) {
                this.ruleForm.shangchuanchuxingbaogao = fileUrls;
				this.addEditUploadStyleChange()
            },
            	addEditStyleChange() {
	  this.$nextTick(()=>{
